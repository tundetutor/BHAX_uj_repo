...
void FrakAblak::vissza(int magassag, int *sor, int meret)
{
    ...
    for(int i=0; i<meret; ++i) 
    {
        ...
        piros = 67 - sor[i]*(sor[i] / 8)*(sor[i] / 8);         //88 - sor[i]*(sor[i]/16);          //106 - sor[i]*sor[i]*sor[i];
        zold = 1024 - sor[i];   //*sor[i]                              //511 - sor[i];                     //512; - sor[i]*sor[i]*sor[i]*sor[i];//*sor[i];
        kek =  222 - sor[i]*(sor[i] / 16);          //188 - sor[i]*(sor[i]/16);         //156 - sor[i]*sor[i]*sor[i];
